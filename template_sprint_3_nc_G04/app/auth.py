    ...
    try:
        ...
        if request.method == 'GET':  # MODIFICADO
            ...
            if attempt is not None:
                ...
                db.execute(
                    'INSERT INTO user (username, password,salt,email) VALUES (?,?,?,?)', (attempt['username'], attempt['password'], attempt['salt'], attempt['email']) # MODIFICADO
                )
                db.commit()
...
@bp.route('/register', methods=('GET', 'POST')) #CAMBIADO MARTES
def register():
    try:
        if g.user:
            return redirect(url_for('inbox.show'))
      
        if request.method == 'POST': # MODIFICADO     
            username = request.form['username'] # MODIFICADO
            password = request.form['password'] # MODIFICADO
            email = request.form['email'] # MODIFICADO
            
            db = get_db() # MODIFICADO
            error = None

            if not username: # MODIFICADO X TUT
            ...
            if db.execute('SELECT id FROM user WHERE username = ?', (username,)).fetchone() is not None: # MODIFICADO
            ...
            if db.execute('SELECT id FROM user WHERE email = ?', (email,)).fetchone() is not None: # MODIFICADO
            ...
            db.execute(
                'INSERT INTO activationlink (challenge, state, username, password,salt,email) VALUES (?,?,?,?,?,?)', # MODIFICADO
...
@bp.route('/confirm', methods=('GET', 'POST')) # MODIFICADO
def confirm():
    try:
        if g.user:
            return redirect(url_for('inbox.show'))
        ...
        if request.method == 'POST':  # MODIFICADO
            password = request.form['password'] # MODIFICADO
            password1 = request.form['password1'] # MODIFICADO
            ...
            if not password1:
                ...
                return render_template('auth/change.html', number=authid) # MODIFICADO

            if password1 != password: # MODIFICADO
            ...
            db = get_db() # MODIFICADO
            attempt = db.execute(
                'SELECT * FROM forgotlink where challenge=? and state=? and CURRENT_TIMESTAMP BETWEEN created AND validuntil', (authid, utils.F_ACTIVE) # MODIFICADO
            ).fetchone()
            
            if attempt is not None:
                db.execute(
                    'UPDATE forgotlink SET state = ? WHERE id = ?', (utils.F_INACTIVE, attempt['id']) # MODIFICADO
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    'UPDATE user SET password = ?, salt=? WHERE id = ?', (hashP, salt, attempt['userid']) # MODIFICADO
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
...
def change():
    try:
        ...
        if request.method == 'GET': # MODIFICADO
            ...
            db = get_db() # MODIFICADO
            attempt = db.execute(
                'SELECT * FROM forgotlink where challenge=? and state=? and CURRENT_TIMESTAMP BETWEEN created AND validuntil', (number, utils.F_ACTIVE) # MODIFICADO
            ).fetchone()
...
def forgot():
    try:
        ...
        if request.method == 'POST':
            ...
            db = get_db()
            user = db.execute(
                'SELECT * FROM user WHERE email = ?', (email,) # MODIFICADO
            ).fetchone()
            ...
            if user is not None:
                ...
                db.execute(
                    'UPDATE forgotlink SET state = ? WHERE userid = ?', # MODIFICADO
                    (utils.F_INACTIVE,user['id'])
                )
                db.execute(
                    'INSERT INTO forgotlink (userid, challenge,state ) VALUES (?,?,?)', # MODIFICADO
...
@bp.route('/login', methods=('GET', 'POST')) # MODIFICADO
def login():
    try:
        if g.user:
            return redirect(url_for('inbox.show'))
        ...
        if request.method == 'POST': # MODIFICADO
            ...
            if error is None:
                session.clear()
                session['user_id'] = user['id'] # MODIFICADO
                return redirect(url_for('inbox.show'))
...
def load_logged_in_user():
    ...
    if user_id is None:
        ...
        g.user = get_db().execute(
            'SELECT * FROM user WHERE id = ?', (user_id,) # MODIFICADO
        ).fetchone()
